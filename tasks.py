...
def docker_compose(context, command, **kwargs):
    ...
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_device_onboarding.project_name}",
        f'--project-directory "{context.nautobot_device_onboarding.compose_dir}"',
    ]
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(os.path.join(context.nautobot_device_onboarding.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_device_onboarding.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_device_onboarding.compose_dir, "creds.env"),
        )
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_device_onboarding.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
@task(
    help={
        "version": "Version of Device Onboarding to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_device_onboarding"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_device_onboarding") / Path("migrations")
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_device_onboarding/templates/"
...
def unittest(  # noqa: PLR0913
    context,
    keepdb=False,
    label="nautobot_device_onboarding",
    failfast=False,
    buffer=True,
    pattern="",
    verbose=False,
